...
import { FaHome, FaExclamationCircle, FaTools, FaUserCircle } from "react-icons/fa";
...
const Navigation = () => {
  ...
  const Menus = [
    ...
    { name: "Profile", icon: FaUserCircle, dis: "translate-x-32", path: "/profile" },
  ];
  ...
  return (
    <div className="bg-[#24272dac] h-[4.4rem] px-6 rounded-xl mb-4 flex items-center">
      <ul className="flex relative list-none">
        {Menus.map((menu, i) => {
          if (auth && menu.name === "Login") return;
          if (!auth && menu.name === "Profile") return;
          return (
